...
function ImageInfo(props) {
    ...
    useEffect(() => {
        let i = props.data.indexOf(props.imageSelected);
        setIndex(i);
    }, [])

    function nextImage(){
        //console.log("next would be: " + (imageIndex + 1))
        if (imageIndex === (props.data.length - 1)) {
            setIndex(0);
        }else{
    ...
    useEffect(() => {
        props.setImageSelected(props.data[imageIndex]);
    }, [imageIndex])
    ...
    return ( 
        <div className="imageInfoContAlone animate__animated animate__fadeIn">
            ...
            <h2 className="photoTitle">{props.imageSelected.title}</h2>
